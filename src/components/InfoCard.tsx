  ...
  return (
    <div className="info-card space-y-4 p-4">
      ...
      <div className="border-t border-gray-800 pt-3">
        ...
        <p className="font-terminal text-xs text-gray-400 leading-relaxed">{whatCondition}</p>
      </div>

      {/* How to Resolve */}
      <div className="border-t border-gray-800 pt-3">
        <div className="font-pixel text-xs text-green-500 tracking-widest mb-2">HOW TO RESOLVE</div>
        <p className="font-terminal text-xs text-gray-400 leading-relaxed">{howToResolve}</p>
      </div>
      ...
      <div className="border-t border-gray-800 pt-4">
        {!inFixMode && fixModes && fixModes.length > 0 ? (
          ...
          <button
            onClick={onAdvanceFix}
            className="w-full px-3 py-2 bg-black text-cyan-400 font-pixel text-xs uppercase tracking-widest border border-cyan-600 hover:border-cyan-400 hover:text-cyan-300 transition-colors"
            style={{
